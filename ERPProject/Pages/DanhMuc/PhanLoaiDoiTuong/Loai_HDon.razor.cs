...
{
    public class Loai_HDonBase : ComponentBase
    {
        public List<loai_hdon> listLoaiHDon { get; set; }
        protected Loai_HDon_CapNhat fCapNhat;
        ...
        protected SfGrid<loai_hdon> gdv;
            ...
            Task.Run(() =>
            {
                InvokeAsync(async () =>
                {
                    ...
                    if (strInfo.Length == 5)
                    {
                        ...
                        await Task.Run(() => { rsModel = ds_phanquyenService.GetQuyenThanhVien(Convert.ToInt32(strInfo[0]), "mnuDM_Loaihoadon"); });
                        _QSD = rsModel.Data;
        ...
        protected async void onTaiLai()
        {
            ...
            var rsModel = new ResultModel<List<loai_hdon>>();
            await Task.Run(() => { rsModel = loai_hdonService.GetAll(); });
            if (rsModel.isThanhCong)
            {
                listLoaiHDon = rsModel.Data;
        ...
        protected void onXoa(int _ID)
        {
            ...
            frmXacNhan.Show("Xóa dòng được chọn ?", "300px", new System.Action(async () =>
            {
                ...
                await Task.Run(() => { rsModel = loai_hdonService.DeleteById(_ID); });
                AppData.loadingPanel.hide();
        ...
        public async Task onXuatExcel()
        {
            ...
            {
                excelExportProperties.DataSource = listLoaiHDon;
            }
