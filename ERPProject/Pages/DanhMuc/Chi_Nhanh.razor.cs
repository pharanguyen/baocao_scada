...
{
    public class Chi_NhanhBase : ComponentBase
    {

        public List<chi_nhanh> listChiNhanh { get; set; }
        protected Chi_Nhanh_CapNhat fCapNhat;
        ...
        protected SfGrid<chi_nhanh> gdv;
        ...
        protected async void onTaiLai()
        {
            ...
            var rsModel = new ResultModel<List<chi_nhanh>>();
            await Task.Run(() => { rsModel = chi_nhanhService.GetAll(); });
            if (rsModel.isThanhCong)
            {
                listChiNhanh = rsModel.Data;
        ...
        protected void onXoa(int _ID)
        {
            ...
            frmXacNhan.Show("Xóa dòng được chọn ?", "300px", new System.Action(async () =>
            {
                ...
                await Task.Run(() => { rsModel = chi_nhanhService.DeleteById(_ID); });
                AppData.loadingPanel.hide();
        ...
        public async Task onXuatExcel()
        {
            ...
            excelExportProperties.FileName = "Danh sach chi nhánhc.xlsx";
            ...
            {
                excelExportProperties.DataSource = listChiNhanh;
            }
