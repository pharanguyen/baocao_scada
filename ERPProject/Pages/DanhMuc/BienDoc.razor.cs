...
{
    public class BienDocBase : ComponentBase
    {
        public List<bien_doc> listBienDoc { get; set; }
        protected BienDoc_CapNhat fCapNhat;
            ...
            Task.Run(() =>
            {
                InvokeAsync(async () =>
                {
                    ...
                    if (strInfo.Length == 5)
                    {
                        ...
                        await Task.Run(() => { rsModel = ds_phanquyenService.GetQuyenThanhVien(Convert.ToInt32(strInfo[0]), "mnuDM_Biendoc"); });
                        _QSD = rsModel.Data;
        ...
        protected async void onTaiLai()
        {
            ...
            var rsModel = new ResultModel<List<bien_doc>>();
            await Task.Run(() => { rsModel = bien_docService.GetAll(); });
        ...
        protected void onXoa(int _ID)
        {
            ...
            frmXacNhan.Show("Xóa dòng được chọn ?", "300px", new System.Action(async () =>
            {
                ...
                await Task.Run(() => { rsModel = bien_docService.DeleteById(_ID); });
                AppData.loadingPanel.hide();
