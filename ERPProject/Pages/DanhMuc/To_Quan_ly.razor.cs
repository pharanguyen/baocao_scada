...
{
    public class To_Quan_lyBase : ComponentBase
    {
        public List<prc_to_quan_ly> listToQuanLy{ get; set; }
        //public List<to_quan_ly> listToQuanLy{ get; set; }
        protected To_Quan_Ly_CapNhat fCapNhat;
        ...
        protected async void onTaiLai()
        {
            ...
            var rsModel = new ResultModel<List<prc_to_quan_ly>>();
            await Task.Run(() => { rsModel = to_quan_lyService.GetAll_prc_to_quan_ly(); });
            if (rsModel.isThanhCong)
            {
                listToQuanLy = rsModel.Data;
        ...
        protected void onXoa(int _ID)
        {
            ...
            frmXacNhan.Show("Xóa dòng được chọn ?", "300px", new System.Action(async () =>
            {
                ...
                await Task.Run(() => { rsModel = to_quan_lyService.DeleteById(_ID); });
                AppData.loadingPanel.hide();
