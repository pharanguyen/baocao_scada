...
{
    public class DmChiNhanhBase : ComponentBase
    {
        public List<DAO.Models.DanhMuc.Dm_ChiNhanh> listBienDoc { get; set; }
        protected Dm_ChiNhanh_CapNhat fCapNhat;
        ...
        protected SfGrid<DAO.Models.DanhMuc.Dm_ChiNhanh> gdv;
            ...
            Task.Run(() =>
            {
                InvokeAsync(async () =>
                {
                    ...
                    if (strInfo.Length == 5)
                    {
                        ...
                        await Task.Run(() => { rsModel = ds_phanquyenService.GetQuyenThanhVien(Convert.ToInt32(strInfo[0]), "btnChiNhanhCN"); });
                        _QSD = rsModel.Data;
        ...
        protected async void onTaiLai()
        {
            ...
            var rsModel = new ResultModel<List<DAO.Models.DanhMuc.Dm_ChiNhanh>>();
            await Task.Run(() => { rsModel = DmChiNhanhService.GetAll(); });
        ...
        protected void onXoa(int _ID)
        {
            ...
            frmXacNhan.Show("Xóa dòng được chọn ?", "300px", new System.Action(async () =>
            {
                ...
                await Task.Run(() => { rsModel = DmChiNhanhService.DeleteById(_ID); });
                AppData.loadingPanel.hide();
