...
{
    public class Quan_CapNhatBase : ComponentBase
    {
        ...
        public quan objModel = new quan();
        public void Show(int _id)
        {
            objID = _id;
            if (!isThemMoi && objID != 0)
            {
                var rsModel = quanService.GetById(_id);
        ...
        {
            if (objModel.ten_quan == null)
            {
                toastService.ShowWarning("Chưa nhập tên quận !");
                // tbHoTen.FocusIn();
                return;
            }
            try
            {
                if (objID == 0)
                {
                    ...
                    await Task.Run(() => { rsModel = quanService.Add(objModel); });
                    if (!rsModel.isThanhCong) throw new Exception(rsModel.ThongBao);
                    toastService.ShowSuccess("Đã thêm quận!");
                    isShow = false;
                }
                else
                {
                    var rsModel = new ResultModel<int?>();
                    await Task.Run(() => { rsModel = quanService.Update(objModel); });
                    if (!rsModel.isThanhCong) throw new Exception(rsModel.ThongBao);
